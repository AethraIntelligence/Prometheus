...
def _load(path: Path) -> Scenario:
    ...
    return Scenario(
        ...
        stop_on_approval=_strings(path, "stop_on_approval", raw.get("stop_on_approval")),
        thread=thread,
...
def _expectations(path: Path, raw: Any) -> Expectations:
    ...
    return Expectations(
        ...
        files_unchanged=_strings(path, "files_unchanged", raw.get("files_unchanged")),
        ...
        employees_involved=_strings(path, "employees_involved", raw.get("employees_involved")),
        results_accepted=bool(raw.get("results_accepted", False)),
